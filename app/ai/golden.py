...
def validate_golden_manifest(
    ...
    if payload.get("label_source") != "operator":
        errors.append("label-source-must-be-operator")
    ...
    for index, raw in enumerate(samples):
        ...
        if raw.get("label_source") != "operator":
            errors.append(f"label-source-must-be-operator:{sample_id}")
            continue
        if raw.get("training_allowed") is not False:
            errors.append(f"training-must-be-forbidden:{sample_id}")
            continue

        expected = normalize_plate(raw.get("expected_plate", ""))
        readable_value = raw.get("readable")
        if not isinstance(readable_value, bool):
            errors.append(f"readable-must-be-boolean:{sample_id}")
            continue
        ...
        if unknown or not labels:
            ...
            continue
        if ("unreadable" in labels) == readable:
            errors.append(f"unreadable-slice-mismatch:{sample_id}")
            continue

        frame_path = str(raw.get("frame_path") or "").strip()
        crop_path = str(raw.get("crop_path") or "").strip()
        if bool(frame_path) == bool(crop_path):
            errors.append(f"exactly-one-media-kind-required:{sample_id}")
            continue
        relative = crop_path or frame_path
        media_kind = "ocr-crop" if crop_path else "frame"
        ...
        if digest in seen_digests:
            errors.append(f"duplicate-media-digest:{sample_id}")
            continue
