...
class LiveANPRWorker:
    ...
    def _overlay_candidates(
        ...
        """Return only visually supported plate boxes for live display.

        Vehicle detection is intentionally not required: at night the plate,
        headlights and local motion can remain visible while the car body is
        not. Static OpenCV rectangles and weak one-frame guesses stay hidden.
        """
        selected = []
        seen = set()
        stable_tracks = {
        ...
        motion_supported = bool(
            activity is None
            or activity.moving
            or activity.scene_change
        )
        for source in list(stable_rows) + list(display_rows):
            row = dict(source)
            ...
            normalized = normalize_plate(
                ...
                or row.get("raw_guess_text")
                or row.get("plate")
            )
            full_text = len(normalized) == 8
            ...
            stable = bool(
                track_id in stable_tracks
                or int(row.get("consensus_votes", 0)) >= 3
            )
            valid = bool(row.get("valid") and full_text)
            reviewable = bool(
                row.get("needs_review")
                or row.get("experimental")
            )

            if method.startswith("opencv") and not (stable and valid):
                continue

            stable_valid = bool(stable and valid)
            stable_review = bool(
                stable
                and reviewable
                and full_text
                and detector_confidence >= 0.34
            )
            strong_valid = bool(
                valid
                and motion_supported
                and combined_confidence
                >= max(0.34, float(min_confidence) * 0.60)
            ...
            strong_review = bool(
                ...
                >= max(0.48, float(min_confidence) * 0.82)
            )
            if not (
                stable_valid
                or stable_review
            ...
            signature = (
                tuple(int(round(value)) for value in bbox),
                normalized,
