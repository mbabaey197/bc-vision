...
class CameraStream:
    ...
    def _publish(self, frame):
        ...
        # Uploaded video may be decoded at 25-30 FPS while the dashboard is
        # encoded at 12 FPS, avoiding both slow playback and wasted CPU.
        ...
        try:
            ...
            # Submit every decoded source frame. LiveANPRWorker keeps only the
            # newest/best pending frame and never blocks the video decoder.
            submit_live_frame(
                self.camera_id,
                self.name,
                frame,
    ...
    def _run_pyav_video(self, source, delay):
        """Decode uploaded video at its native clock without blocking ANPR."""
        if not AV_OK:
            raise RuntimeError(
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                while not self.stop_event.is_set():
                    ...
                    # The live worker owns a latest-frame queue, so stale AI
                    # frames are dropped instead of slowing the video clock.
                    self._publish(frame)
                    published += 1
