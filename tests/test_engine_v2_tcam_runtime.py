...
def test_runtime_locks_after_independent_confirmation_and_suppresses_more_ocr() -> None:
    ocr = _OCR(0.87)
    engine = _engine(ocr)
    ...
    assert engine.submit_frame(FramePacket("cam", 4, 4.0, _frame(110))) is True
    event = engine.process_next()
    assert event is not None
    assert event.text == PLATE
    assert event.metadata["recognition_phase"] == "locked"
    ...
    assert telemetry["fusion_locked_tracks"] == 1
    assert telemetry["fusion_ocr_attempts"] == 2

    assert engine.submit_frame(FramePacket("cam", 5, 5.0, _frame(115))) is True
    assert engine.process_next() is None
    assert ocr.calls == 2
