...
def test_strict_first_frame_express_path_can_lock() -> None:
    fusion = PlateEvidenceAccumulator()

    decision = fusion.add(_result(PLATE, 0.96), quality=0.85, seq=1)

    assert decision.phase is RecognitionPhase.LOCKED
...
def test_partial_slots_remain_open_and_are_filled_by_later_frames() -> None:
    ...
    assert complete.phase is RecognitionPhase.LOCKED
...
def test_correlated_adjacent_frames_do_not_fake_independent_consensus() -> None:
    ...
    assert third.phase is RecognitionPhase.LOCKED
...
def test_scheduler_rereads_on_area_growth_and_stops_after_lock() -> None:
    session = TrackRecognitionSession()
    ...
    assert session.should_schedule_ocr(seq=10, quality=1.0, bbox_area=4_000).reason == (
        "recognition_locked"
    )


def test_one_locked_track_can_claim_only_one_event() -> None:
    session = TrackRecognitionSession()
